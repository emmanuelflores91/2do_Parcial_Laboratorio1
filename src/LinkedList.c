    	...
    	for(int i = 0; i<ll_len(this)-1; i++)
    	{
    		for(int j = i+1; j<ll_len(this); j++)
    		{
    			...
    			if(pElement1!= NULL && pElement2 != NULL &&
    			  ((pFunc(pElement1,pElement2)==1 && order == 1)||(pFunc(pElement1,pElement2)==-1 && order == 0)))
    			{
...
LinkedList* ll_filter(LinkedList* lista, int(*fn)(void*))
{
...
int ll_count(LinkedList* lista, int (*fn) (void*))
{
	int acumulador = -1;
	int retornoFn;
	...
	for (int i = 0; i < ll_len(lista); i++)
	{
		...
		if (pElement != NULL && retornoFn>0)
		{
			acumulador = 0;

			acumulador += retornoFn;
		}
	}
	return acumulador;
}
